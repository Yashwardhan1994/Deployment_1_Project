...
**Loop for counting all the Category and Numerical features values in Dataset**
...
data.isnull().sum()
**Checking all the datatypes across the dataset**
data.dtypes
**UNIVARIATE DATA ANALYSIS BY HISTOGRAM AND COUNTPLOT GRAPHS**
...
**BIIVARIATE ANALYSIS USING BARPLOT AND SCATTER PLOT GRAPHS**
#Loop for plotting all the count plot of all the Category and object features.
for column in data.select_dtypes(include=['int', 'float']).columns:
    ...
    plt.xticks(rotation=45)  # Rotate x-axis labels if necessary
    plt.show()
**From the above BoxPlots graphs, it is evident that will all the integer features except days classification of Target variable will be weak due to the presence of Outliers in each numerical features with respect to the Target variable (Subscibed)**
** **
import matplotlib.pyplot as plt
import seaborn as sns
...
The majority of individuals do not subscribe, indicating room for improvement in targeting the customers.
Features like age (28-60), day (uniform thoroughout the month), campaign (1-5 prior campaign contacted customer are likely to subscibed) and prior interaction (1-3 are critical) are  factors influencing subscription
**  **
import matplotlib.pyplot as plt
import seaborn as sns
...
High noise in this scatterplots Graphs, indicates that any 2 Numerical feature will be weak predictors of subscription status when considered alone.
** **
MULTIVARIATE 3-D SCATTER PLOT
...
** **
**Finding Mode , Median for Imputing Missing Values in across the dataset**
